			...
			try {
				...
				if((!fechaString.equalsIgnoreCase("")) || (!fechaInicioString.equalsIgnoreCase("")) || (!fechaFinalString.equalsIgnoreCase(""))) {
					...
					try {
						...
						if((modificado.getFechaInicio().compareTo(modificado.getFechaPublicacion()) == 0) && (modificado.getFechaInicio().before(modificado.getFechaFinal()))){
	
							modificado.setFase(Fases.publicado);
							modificado.setFechaPublicacion(new Date());
	
			            }
						else if(modificado.getFechaInicio().before(modificado.getFechaPublicacion())){
							
							modificado.setFase(Fases.espera);
						}
						else if(modificado.getFechaFinal().before(fechaActual)) {
							

							modificado.setFase(Fases.archivado);
						}
